...
public class UCSMazeSolver3D {
	...
	public UCSMazeSolver3D(Maze3D maze){
		this.maze = maze;
		...
		while(!priorityQueue.isEmpty()){
			...
			for (int i = 0; i < this.AdjacentCells.size(); i++){
				if (this.AdjacentCells.get(i).IsEnd()){
					this.AdjacentCells.get(i).setTotalCost(cell.getTotalCost() + cell.getSteps());
